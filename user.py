    ...
    def step(self, arms_reward_old, num_users_on_channels):
        ...
        if self.a[t-1] >= 0:
            ...
            # else, get reward
                ...
                # update ˆμa(t−1), get reward if any
        ...
        if not len(free_arms):
            self.a[t] = -1
            return None # Refrain from transmitting in this round
        
        ################ explore or exploit ################
        epsilon = np.min([1, (c*K**2)/(d**2 * (K-1) *t)])
        if self.a[t-1] < 0 or np.random.rand() <= epsilon:
            # explore
...
def run_simulation(p_0, alpha, beta, c, d, K, time_end, N, mu_list):
...
def test_1():
    # mu_list = [0.3, 0.7 ,0.4, 0.3, 0.7 ,0.4, 0.3, 0.7 ,0.4]
    mu_list = [1,1,1,1,1,1,1,1,1]
    K = len(mu_list)
    N = 6
    c = 0.1
    p_0 = 0.6
    ...
    num_users_on_channels, users = run_simulation(p_0, alpha, beta, c, d, K, time_end, N, mu_list)

    # plot
    all_collision = np.sum(np.maximum(0, num_users_on_channels - 1), axis=1)
    collision_over_time = np.cumsum(all_collision)
    plt.plot(collision_over_time)
